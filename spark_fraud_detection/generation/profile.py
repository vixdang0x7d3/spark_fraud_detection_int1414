...
class Profile:
    def __init__(self, profile, seed=42):
        ...
        self.proportions["categories_wt"] = self._weight_to_cumsum(
            self.profile["categories_wt"]
        )
        self.proportions["shopping_time"] = self._weight_to_cumsum(
            self.profile["shopping_time"]
        )  ###BRANDON
        self.proportions["date_wt"] = {}
        self.proportions["date_wt"]["day_of_week"] = self._prep_weekday()
        years_wt, leap_wt = self._prep_holidays()
        self.proportions["date_wt"]["time_of_year"] = years_wt
        self.proportions["date_wt"]["time_of_year_leap"] = leap_wt
        self.amt_specs = self._pre_compute_amt_specs()
    ...
    def sample_from(self, is_fraud):
        ...
        rnds = self._get_rand_2d(num_trans, 2, 4)

        rnds = self._closest_rand_parallel(rnds, 1, 3, self.proportions["date_prop"])
        rnds = self._closest_rand_parallel(
            rnds, 2, 5, self.proportions["shopping_time"]
        )
        rnds = self._closest_rand_parallel(
            rnds, 2, 4, self.proportions["categories_wt"]
        )
        ...
        for i, cat_prop in enumerate(unique):
            ...
            rnds[offset : offset + counts[i], 6] = np.where(
                rnd_amts < 1, rnd_amts_lower, rnd_amts
            )
            offset += counts[i]
        ...
        for i in range(num_trans):
            ...
            hr, mn, sec = self._sample_time(chosen_daypart, is_fraud)
            chosen_date = datetime.combine(
                chosen_date, time(hour=hr, minute=mn, second=sec)
            )
    ...
    def _weight_to_cumsum(self, weights):
    ...
    def _date_weights(self):
        self.proportions["date_wt"]["year"] = self._prep_years()
        weights = self._combine_date_params(self.proportions["date_wt"])
        self.proportions["date_prop"] = self._weight_to_cumsum(weights)

    def _combine_date_params(self, weights):
        new_date_weights = {}
        curr = self.start
        while curr <= self.end:
            # leap year:
        ...
        return self._weight_to_prop(new_date_weights)

    def _prep_weekday(self):
        day_map = {
            "monday": 0,
            "tuesday": 1,
            "wednesday": 2,
        ...
        return self._weight_to_prop(weekdays)
    ...
    def _prep_holidays(self):
        ...
        return self._weight_to_prop(days_nonleap), self._weight_to_prop(days)

    def _prep_years(self):
        final_year = {}
        ...
        for i, y in enumerate(years):
            ...
            # if not enough years provided, make it 100
                ...
                final_year[y] = 100
        return self._weight_to_prop(final_year)
    ...
    def _weight_to_prop(self, weights):
        wt_tot = sum(weights.values())
        return {k: weights[k] / float(wt_tot) for k in weights.keys()}

    def _pre_compute_amt_specs(self):
        amt_specs = {}
    ...
    def _sample_time(self, am_or_pm, is_fraud):
        if am_or_pm == "AM":
            hr_start = 0
            hr_end = 12
        if am_or_pm == "PM":
    ...
    @staticmethod
    def _closest_rand_parallel(r, i, j, obj):
